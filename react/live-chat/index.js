...
const rooms = [new ChatRoom(chatServer, 'Test')]
chatServer.on('clientMessage', (client, msg) => {
    ...
    switch (msg.type) {
        case SET_NAME:
            ...
            break
        case SEND_MESSAGE:
            client.room.message.push({ from: client.name, content: msg.payload })
        ...
        case JOIN_ROOM:
            ...
            client.room.broadcast(new Message (USER_LIST, ''))
            client.send(new Message(MESSAGE_LIST, client.room.message.map(({ from, content }) => ({ from, content }))))
            break;
    }
})
